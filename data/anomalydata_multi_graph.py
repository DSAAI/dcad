...
class MultiGraphAnomalyDataSet(InMemoryDataset):
    def __init__(self,root,transform=None, pre_transform=None):
        super(MultiGraphAnomalyDataSet, self).__init__(root, transform, pre_transform)  # transform就是数据增强，对每一个数据都执行
    ...
    def processed_file_names(self):  # 检查self.processed_dir目录下是否存在self.processed_file_names属性方法返回的所有文件，没有就会走process
        return ['multi_graph_anomaly.dataset']

    def download(self):
        pass

    def process(self):
        csv_file = "/home/cjl/csv/anomalydata.csv"
