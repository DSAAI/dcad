...
class TraceTestDataSet(InMemoryDataset):
    def __init__(self,root, transform=None, pre_transform=None):
        super(TraceTestDataSet, self).__init__(root, transform, pre_transform)  # transform就是数据增强，对每一个数据都执行
    ...
    def process(self):
        ...
        for traceId, group in tqdm(grouped):
            ...
            for element in range(0, len(rawNetworklatency)):
                edge_feature.append(
                    [rawNetworklatency[element], rawProcessingTime[element], proportionProcessingTime[element],
                     rawDuration[element], isError[element], workDuration[element],
